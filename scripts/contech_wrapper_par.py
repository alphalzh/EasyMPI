...
def main(isCpp = False, markOnly = False, minimal = False, hammer = False):
    ...
    # Requires dragon egg
        ...
        pcall(["gcc", CFLAGS, "-S", "-fplugin=" + CONTECH_LLVM_HOME + "/lib/dragonegg.so" , "-fplugin-arg-dragonegg-emit-ir", cfile, "-o", newobj])
    ...
    if not compileOnly:
        if ofiles != "":
            ...
            if ARM == True:
                ...
                pcall([CC, "-flto", oAltLib, out + "_ct.link.bc", oAltLib, RUNTIME, "--verbose" ,CFLAGS, "-o", out, "-lrt", "-ldl", "-lpthread",  "contech_state.o"])
        else:
            print "Pass through"
            passThrough(CC)
